...
function formatForecastDayLabel(dateString, offset) {
  ...
  return new Date(dateString).toLocaleDateString(locale, { weekday: "short", month: "short", day: "numeric" });
}

function getCurrentHourlyIndex(data) {
  const times = data.hourly?.time;
  if (!times?.length) return -1;
  const anchor = data.current?.time;
  if (!anchor) return times.length - 1;
  const hourKey = anchor.slice(0, 13);
...
function applyOpenMeteoSnapshot(data) {
  ...
  if (hourlyIndex >= 0 && hourly) {
    if (hourly.temperature_2m?.[hourlyIndex] != null) state.metrics.temp = Math.round(hourly.temperature_2m[hourlyIndex]);
    if (hourly.relative_humidity_2m?.[hourlyIndex] != null) state.metrics.humidity = Math.round(hourly.relative_humidity_2m[hourlyIndex]);
    if (hourly.wind_speed_10m?.[hourlyIndex] != null) state.metrics.wind = Math.round(hourly.wind_speed_10m[hourlyIndex]);
  } else if (current) {
  ...
  } else if (hourlyIndex >= 0 && hourly?.precipitation_probability?.[hourlyIndex] != null) {
    state.metrics.rainChance = Number(hourly.precipitation_probability[hourlyIndex]);
  }

  if (daily?.weather_code?.[todayIndex] != null) {
    state.metrics.weatherCode = Number(daily.weather_code[todayIndex]);
  } else if (hourlyIndex >= 0 && hourly?.weather_code?.[hourlyIndex] != null) {
    state.metrics.weatherCode = Number(hourly.weather_code[hourlyIndex]);
  } else if (current) {
    state.metrics.weatherCode = Number(current.weather_code ?? state.metrics.weatherCode ?? 0);
  }

  if (daily?.precipitation_sum?.[todayIndex] != null) {
    state.metrics.rain = Number(Number(daily.precipitation_sum[todayIndex]).toFixed(1));
  } else if (current) {
    state.metrics.rain = Number((current.rain ?? current.precipitation ?? state.metrics.rain).toFixed(1));
  }
}
...
function buildForecastFromOpenMeteo(data) {
  ...
  for (let offset = 0; offset < 7; offset += 1) {
    ...
    const tempLabel = min != null ? `${Math.round(max)}° / ${Math.round(min)}°` : `${Math.round(max)}°`;
    items.push([
      formatForecastDayLabel(daily.time[index], offset),
      weatherCodeToIcon(daily.weather_code[index]),
      tempLabel,
      `${Math.round(daily.precipitation_probability_max[index] ?? 0)}%`
    ]);
  }
  forecast = items;
  return forecast;
...
function buildManualForecast() {
  ...
  forecast = Array.from({ length: 7 }, (_, index) => {
    ...
    const dayTemp = Math.round(temp + Math.min(index, 2) - Math.max(0, index - 3));
    return [
      index === 0 ? t("today") : date.toLocaleDateString(locale, { weekday: "short", month: "short", day: "numeric" }),
      weatherCodeToIcon(chance > 55 ? 61 : chance > 25 ? 3 : index % 2 === 0 ? 1 : 2),
      `${dayTemp}°`,
      `${chance}%`
    ];
  });
...
function applyKazhydrometForecast(data) {
  ...
  if (Array.isArray(data.forecast) && data.forecast.length) {
    forecast = data.forecast.slice(0, 7).map((item, index) => [
      index === 0 ? t("today") : (item.day || item.label || new Date(item.date || Date.now()).toLocaleDateString(locale, { weekday: "short", month: "short", day: "numeric" })),
      item.icon || weatherCodeToIcon(item.code ?? item.weatherCode ?? 0),
      `${Math.round(item.temp ?? item.temperature ?? state.metrics.temp)}°`,
      `${Math.round(item.rainChance ?? item.rain ?? item.precipitationProbability ?? 0)}%`
    ]);
    return;
  }
...
function renderForecast(targetId) {
  ...
  if (!target) return;
  target.innerHTML = forecast.slice(0, 7).map(([day, icon, temp, rain]) => `
...
function applyLanguage() {
  ...
  if (lastForecastData?.daily?.time?.length && state.provider === "openmeteo") {
...
function applyMetrics() {
  ...
  const headerRainChance = todayForecast?.[3] != null
    ? Number(String(todayForecast[3]).replace("%", ""))
    : Number(metrics.rainChance || 0);
  const headerIcon = todayForecast?.[1] ?? weatherCodeToIcon(Number(metrics.weatherCode ?? 0));
...
async function fetchGoogleForecast(region) {
  ...
  forecast = (data.forecastDays || []).slice(0, 7).map((day, index) => {
    ...
    const weatherType = day.daytimeForecast?.weatherCondition?.type
      ...
      ?? "";
    const code = /RAIN|SHOWER|DRIZZLE/i.test(weatherType) ? 61 : /CLOUD/i.test(weatherType) ? 3 : 0;
    return [
      index === 0 ? t("today") : date.toLocaleDateString(locale, { weekday: "short", month: "short", day: "numeric" }),
      weatherCodeToIcon(code),
      `${Math.round(index === 0 ? state.metrics.temp : (day.maxTemperature?.degrees ?? day.temperature?.degrees ?? state.metrics.temp))}°`,
