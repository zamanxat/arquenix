...
const GISMETEO_API = "http://45e30b7f.services.gismeteo.ru/inform-service/a407a91cfcb53e52063b77e9e777f5bd";
...
const needsProviderMigration = (state.weatherSourceVersion || 1) < 2;
normalizeState();
if (needsProviderMigration) saveState();
...
function normalizeState() {
  ...
  Object.entries(gismeteoIds).forEach(([key, id]) => {
    if (regions[key]) regions[key].gismeteoId = id;
  });
  const validProviders = ["gismeteo", "openmeteo", "google", "kazhydromet", "manual"];
  if (!validProviders.includes(state.provider)) state.provider = "gismeteo";
  if ((state.weatherSourceVersion || 1) < 2) {
    state.provider = "gismeteo";
    state.weatherSourceVersion = 2;
  }
}
...
function dailyForecastIcon(code, rainPct) {
  if (Number(rainPct) < 20 && [51, 53, 55, 61, 63, 65, 80, 81, 82].includes(code)) return "☁";
  return weatherCodeToIcon(code);
}

function getGismeteoCityId(regionKey) {
  const region = regions[regionKey] || regions.almaty;
  return region.gismeteoId || gismeteoIds.almaty;
}

function parseXmlAttrs(source) {
  const attrs = {};
  if (!source) return attrs;
  [...source.matchAll(/(\w+)="([^"]*)"/g)].forEach((match) => {
    attrs[match[1]] = match[2];
  });
  return attrs;
}

function parseGismeteoXml(xml) {
  if (xml.includes("errorCode")) {
    throw new Error("Gismeteo: қала дерегі табылмады");
  }
  const factMatch = xml.match(/<fact[\s\S]*?<values\s+([^>]+?)(?:\/>|><\/values>)/);
  const current = parseXmlAttrs(factMatch?.[1]);
  const days = [...xml.matchAll(/<day\s+([^>]+)>/g)]
    .map((match) => parseXmlAttrs(match[1]))
    .slice(0, 7);
  return { current, days };
}

function gismeteoIconToCode(icon) {
  const value = String(icon || "").toLowerCase();
  if (value.includes("st") || value.includes("rs")) return 95;
  if (value.includes("r3")) return 65;
  if (value.includes("r2")) return 61;
  if (value.includes("r1")) return 51;
  if (value.includes("s")) return 71;
  if (value.includes("c3")) return 3;
  if (value.includes("c2")) return 2;
  if (value.includes("c1")) return 1;
  return 0;
}

function gismeteoPrToRain(pr) {
...
function applyGismeteoSnapshot(data) {
  const current = data.current || {};
  const today = data.days?.[0] || {};
  state.metrics.temp = Math.round(Number(current.t || state.metrics.temp));
  state.metrics.humidity = Math.round(Number(current.hum || state.metrics.humidity));
  state.metrics.wind = Math.round(Number(current.ws || 0) * 3.6);
  state.metrics.weatherCode = gismeteoIconToCode(current.icon);
  state.metrics.rainChance = gismeteoPrToRain(today.pr);
  state.metrics.rain = Number(today.prflt || current.prflt || current.pr || state.metrics.rain || 0);
}

function buildForecastFromGismeteo(data) {
  if (!data?.days?.length) return buildManualForecast();
  forecast = data.days.slice(0, 7).map((day, index) => [
    formatForecastDayLabel(day.date, index),
    gismeteoIconToCode(day.icon),
    Math.round(Number(day.tmin)),
    Math.round(Number(day.tmax)),
    gismeteoPrToRain(day.pr)
  ]);
  return forecast;
}

function dailyForecastLucide(code, rainPct) {
  const value = Number(code);
  if (Number(rainPct) < 20 && [51, 53, 55, 61, 63, 65, 80, 81, 82].includes(value)) return "cloud";
  if ([0, 1].includes(value)) return "sun";
  if (value === 2) return "cloud-sun";
  if (value === 3) return "cloud";
  if ([45, 48].includes(value)) return "cloud-fog";
  if ([51, 53, 55, 61, 63, 65, 80, 81, 82].includes(value)) return "cloud-rain";
  if ([71, 73, 75, 77, 85, 86].includes(value)) return "snowflake";
  if ([95, 96, 99].includes(value)) return "cloud-lightning";
  return "cloud";
}

function getLocalTodayKey(referenceTime, timezone) {
  if (referenceTime) return String(referenceTime).slice(0, 10);
  if (timezone) {
    try {
...
function getTodayDailyIndex(times, referenceTime, timezone) {
  const today = getLocalTodayKey(referenceTime, timezone);
  const index = times.findIndex((time) => String(time).slice(0, 10) >= today);
  return index === -1 ? 0 : index;
}

function formatForecastDayLabel(dateString, offset) {
...
function applyOpenMeteoSnapshot(data) {
  const current = data.current;
  const daily = data.daily;
  const todayIndex = getTodayDailyIndex(daily?.time || [], data.current?.time, data.timezone);

  if (current) {
    state.metrics.temp = Math.round(current.temperature_2m);
    state.metrics.humidity = Math.round(current.relative_humidity_2m);
    state.metrics.wind = Math.round(current.wind_speed_10m);
    state.metrics.weatherCode = Number(current.weather_code ?? 0);
  }

  if (daily?.precipitation_probability_max?.[todayIndex] != null) {
    state.metrics.rainChance = Number(daily.precipitation_probability_max[todayIndex]);
  }

  if (daily?.precipitation_sum?.[todayIndex] != null) {
    state.metrics.rain = Number(Number(daily.precipitation_sum[todayIndex]).toFixed(1));
  } else if (current) {
    state.metrics.rain = Number((current.rain ?? current.precipitation ?? 0).toFixed(1));
  }
}

function buildForecastFromOpenMeteo(data) {
  const daily = data.daily;
  if (!daily?.time?.length) return buildManualForecast();
  const start = getTodayDailyIndex(daily.time, data.current?.time, data.timezone);
  const items = [];
  for (let offset = 0; offset < 7; offset += 1) {
    const index = start + offset;
    if (index >= daily.time.length) break;
    const max = Math.round(daily.temperature_2m_max[index]);
    const min = Math.round(daily.temperature_2m_min?.[index] ?? max - 8);
    const rain = Math.round(daily.precipitation_probability_max[index] ?? 0);
    items.push([
      formatForecastDayLabel(daily.time[index], offset),
      Number(daily.weather_code[index] ?? 0),
      min,
      max,
      rain
    ]);
...
function buildManualForecast() {
  const locale = state.language === "ru" ? "ru-RU" : "kk-KZ";
  const temp = Number(state.metrics.temp || 18);
  const rainChance = Number(state.metrics.rainChance || 40);
  forecast = Array.from({ length: 7 }, (_, index) => {
    const date = new Date();
    date.setDate(date.getDate() + index);
    const chance = clamp(rainChance - index * 4, 0, 100);
    const dayHigh = Math.round(temp + Math.min(index, 2) - Math.max(0, index - 3));
    const dayLow = Math.round(dayHigh - 8);
    const code = chance > 55 ? 61 : chance > 25 ? 3 : index % 2 === 0 ? 1 : 2;
    return [
      index === 0 ? t("today") : date.toLocaleDateString(locale, { weekday: "short" }),
      code,
      dayLow,
      dayHigh,
      chance
...
function applyKazhydrometForecast(data) {
  const locale = state.language === "ru" ? "ru-RU" : "kk-KZ";
  if (Array.isArray(data.forecast) && data.forecast.length) {
    forecast = data.forecast.slice(0, 7).map((item, index) => {
      const high = Math.round(item.max ?? item.temp ?? item.temperature ?? state.metrics.temp);
      const low = Math.round(item.min ?? high - 8);
      const rain = Math.round(item.rainChance ?? item.rain ?? item.precipitationProbability ?? 0);
      return [
        index === 0 ? t("today") : (item.day || item.label || new Date(item.date || Date.now()).toLocaleDateString(locale, { weekday: "short" })),
        Number(item.code ?? item.weatherCode ?? 0),
        low,
        high,
        rain
      ];
  ...
  if (data.daily?.time?.length) {
    buildForecastFromOpenMeteo(data);
    return;
  }
  buildManualForecast();
}
...
function renderForecast(targetId) {
  ...
  const lows = items.map((entry) => Number(entry[2]));
  const highs = items.map((entry) => Number(entry[3]));
  const globalMin = Math.min(...lows);
  const globalMax = Math.max(...highs);
  const span = globalMax - globalMin || 1;
  const currentTemp = Number(state.metrics.temp);

  target.innerHTML = items.map(([day, code, min, max, rain], index) => {
    const low = Number(min);
    const high = Number(max);
    const rainValue = Number(rain);
    const left = ((low - globalMin) / span) * 100;
    const width = Math.max(((high - low) / span) * 100, 8);
    const lucide = dailyForecastLucide(code, rainValue);
    const rainHtml = rainValue > 0
      ? `<span class="forecast-rain-pct">${rainValue}%</span>`
      : `<span class="forecast-rain-pct forecast-rain-pct--empty" aria-hidden="true"></span>`;
    const nowLeft = clamp(((currentTemp - globalMin) / span) * 100, 0, 100);
    const dotHtml = index === 0 ? `<b class="forecast-now-dot" style="left:${nowLeft}%"></b>` : "";

    return `
      <div class="forecast-day forecast-day--list">
        <span class="forecast-day-label">${day}</span>
        <div class="forecast-icon-wrap">
          <i data-lucide="${lucide}" class="forecast-lucide" aria-hidden="true"></i>
          ${rainHtml}
        </div>
        <span class="forecast-min">${low}°</span>
        <div class="forecast-bar">${dotHtml}<i style="left:${left}%;width:${width}%"></i></div>
        <span class="forecast-max">${high}°</span>
      </div>
    `;
  }).join("");
  renderAiSuggestions();
...
function getTomorrowRainChance() {
  ...
  return Number(forecast[1][4] ?? state.metrics.rainChance ?? 0) || 0;
}
...
function applyLanguage() {
  ...
  if (lastGismeteoData?.days?.length && state.provider === "gismeteo") {
    buildForecastFromGismeteo(lastGismeteoData);
  } else if (lastForecastData?.daily?.time?.length && state.provider === "openmeteo") {
    buildForecastFromOpenMeteo(lastForecastData);
  } else if (state.provider === "manual") {
    buildManualForecast();
  }
...
function setSelectOptionLabels() {
  ...
  if (provider) {
    provider.options[0].textContent = "Gismeteo";
    provider.options[1].textContent = ru ? "Точные данные: Open-Meteo" : "Нақты дерек: Open-Meteo";
    provider.options[2].textContent = "Google Weather API";
    provider.options[3].textContent = "Kazhydromet endpoint";
    provider.options[4].textContent = ru ? "Ручной ввод" : "Қолмен енгізу";
  }
...
function applyMetrics() {
  ...
  setText("#weatherIcon", weatherCodeToIcon(Number(metrics.weatherCode ?? 0)));
  setText("#humidityTop", `${Math.round(metrics.humidity)}%`);
  setText("#windTop", `${formatNumber(metrics.wind)} ${t("unitSpeed")}`);
  setText("#rainChanceTop", `${Number(metrics.rainChance || 0)}%`);
...
async function refreshWeather(options = {}) {
  ...
  try {
    if (state.provider === "gismeteo") {
      try {
        ...
        if (!silent) {
          const ru = state.language === "ru";
          showNotice(
            ru ? "Gismeteo недоступен" : "Gismeteo қосылмады",
            ru
              ? "Показаны данные Open-Meteo. Перезапустите Live Server (Go Live), чтобы включить proxy."
              : "Open-Meteo дерегі көрсетілді. Live Server-ді қайта іске қосыңыз (Go Live)."
          );
        }
      }
    }
    ...
    renderForecast("forecastRow");
    renderForecast("forecastLarge");
    if (!silent) addJournal(t("journalWeather"));
  } catch (error) {
    if (!silent) showNotice(t("weatherError"), error.message || t("apiCheck"));
  }
}

function getGismeteoRequestUrls(cityId, lang) {
  const query = `forecast?lang=${lang}&city=${cityId}`;
  const urls = [`/gismeteo-api/${query}`];
  if (state.endpoint) {
    urls.push(`${state.endpoint.replace(/\/$/, "")}/${query}`);
  }
  urls.push(`http://127.0.0.1:8787/${query}`);
  urls.push(`${GISMETEO_API}/${query}`);
  return [...new Set(urls)];
}

async function fetchGismeteoXml(cityId, lang) {
  const headers = { "User-Agent": "Gismeteo Android v2, RainGrid/1.0" };
  let lastError = null;
  for (const url of getGismeteoRequestUrls(cityId, lang)) {
    try {
      const response = await fetch(url, { headers });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const xml = await response.text();
      if (!xml.includes("<weather>")) throw new Error("Invalid Gismeteo response");
      return xml;
    } catch (error) {
      lastError = error;
    }
...
async function fetchGismeteo() {
  const cityId = getGismeteoCityId(state.region);
  const lang = state.language === "ru" ? "ru" : "ru";
  const data = parseGismeteoXml(await fetchGismeteoXml(cityId, lang));
  lastGismeteoData = data;
  lastForecastData = null;
  applyGismeteoSnapshot(data);
  applyClimateProfile();
  buildForecastFromGismeteo(data);
}
...
async function fetchOpenMeteo() {
  ...
  url.search = new URLSearchParams({
    ...
    current: "temperature_2m,relative_humidity_2m,wind_speed_10m,precipitation,rain,weather_code,is_day",
  ...
  lastGismeteoData = null;
  applyOpenMeteoSnapshot(data);
  applyClimateProfile();
  buildForecastFromOpenMeteo(data);
}
...
async function fetchGoogleWeather() {
  ...
  state.metrics.weatherCode = /THUNDER/i.test(weatherType) ? 95 : /RAIN|SHOWER|DRIZZLE/i.test(weatherType) ? 61 : /CLOUD/i.test(weatherType) ? 3 : 0;
  applyClimateProfile();
  await fetchGoogleForecast(region);
}

async function fetchGoogleForecast(region) {
  ...
  const response = await fetch(url);
  if (!response.ok) throw new Error("Google болжам API сұранысы орындалмады.");
  const data = await response.json();
  const locale = state.language === "ru" ? "ru-RU" : "kk-KZ";
  forecast = (data.forecastDays || []).slice(0, 7).map((day, index) => {
    const display = day.displayDate || {};
    const date = new Date(display.year, (display.month || 1) - 1, display.day || 1);
    const rainChance = day.daytimeForecast?.precipitation?.probability?.percent
      ?? day.precipitation?.probability?.percent
      ?? 0;
    const weatherType = day.daytimeForecast?.weatherCondition?.type
      ?? day.weatherCondition?.type
      ?? "";
    const code = /THUNDER/i.test(weatherType) ? 95 : /RAIN|SHOWER|DRIZZLE/i.test(weatherType) ? 61 : /CLOUD/i.test(weatherType) ? 3 : 0;
    const high = Math.round(day.maxTemperature?.degrees ?? day.temperature?.degrees ?? state.metrics.temp);
    const low = Math.round(day.minTemperature?.degrees ?? high - 8);
    return [
      index === 0 ? t("today") : date.toLocaleDateString(locale, { weekday: "short" }),
      code,
      low,
  ...
  if (!forecast.length) buildManualForecast();
}
...
function sliceMeteoSeries(data) {
  const today = getLocalTodayKey(data.current?.time, data.timezone);
...
if (state.provider !== "manual") {
  refreshWeather();
  setInterval(() => refreshWeather({ silent: true }), 15 * 60 * 1000);
} else {
  buildManualForecast();
  fetchCityCharts();
}
